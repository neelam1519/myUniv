...
//   const LeaveFormHome({super.key});
//
//   @override
//   State<LeaveFormHome> createState() => _LeaveFormHomeState();
// }
//
...
//       ),
//       body: user == null
//           ? const Center(child: Text('Please log in to view your leave history'))
...
//             return const Center(child: Text('No leave forms found.'));
//           }
//
...
//                     children: [
//                       Text('$studentId', style: const TextStyle(fontWeight: FontWeight.bold)),
...
import '../provider/leaveformshome_provider.dart';
...
class _LeaveFormHomeState extends State<LeaveFormHome> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Consumer<LeaveFormsHomeProvider>(
        builder: (context, provider, child) {
          ...
          return user == null
              ? const Center(child: Text('Please log in to view your leave history'))
              : FutureBuilder<Map<String, Map<String, dynamic>>>(
                  future: provider.getLeaveDetails(),
                  builder: (context, snapshot) {
                    if (snapshot.connectionState == ConnectionState.waiting) {
                      return const Center(child: CircularProgressIndicator());
                    }
                    ...
                    return ListView.builder(
                      ...
                      itemBuilder: (context, index) {
                        ...
                        final approvalStatus = leaveForm['finalApproval']['status'] == true ? 'Approved' : 'Pending';
                        final createdAt =
                            leaveForm['createdAt'] != null ? (leaveForm['createdAt'] as Timestamp).toDate() : null;
                          ...
                          child: Padding(
                            ...
                            child: Column(
                              ...
                              children: [
                                ...
                                if (createdAt != null)
                                  Text(
                                      'Submitted on ${createdAt.toLocal().toString().split(' ')[0]} at ${createdAt.toLocal().hour}:${createdAt.toLocal().minute.toString().padLeft(2, '0')}'),
                              ],
                            ),
                          ),
                        );
      ...
      floatingActionButton: FloatingActionButton(
        onPressed: () {
          Provider.of<LeaveFormsHomeProvider>(context, listen: false).leaveFormApply(context);
        },
        child: const Icon(Icons.add),
      ),
    );
